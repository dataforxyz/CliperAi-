...
class FaceReframingSpike:
    def __init__(self, frame_sample_rate: int = 3):
        ...
        self.face_detector = self.mp_face_detection.FaceDetection(
            model_selection=1, min_detection_confidence=0.5  # Full-range model
        )
    ...
    def _generate_report(self, detections: list[dict], fps: float):
        ...
        avg_time = sum(self.stats["detection_time_ms"]) / len(
            self.stats["detection_time_ms"]
        )
        frames_processed = self.stats["frames_processed"]
        frames_with_faces = self.stats["frames_with_faces"]
        face_rate = (
            (frames_with_faces / frames_processed * 100) if frames_processed > 0 else 0
        )

        # Movement analysis
        if len(detections) > 1:
            movements = [
                abs(detections[i + 1]["center_x"] - detections[i]["center_x"])
