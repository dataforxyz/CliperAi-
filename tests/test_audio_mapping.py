...
def test_ffmpeg_command_with_audio_mapping():
    ...
    cmd_correct = [
        ...
        # Mapear streams
        "-map",
        "[v]",  # Video procesado (con subtítulos)
        "-map",
        "1:a",  # Audio del video original [1]
        # Codecs
        "-c:v",
        "libx264",
        "-c:a",
    ...
    cmd_incorrect = [
        "ffmpeg",
        "-i",
        str(video_reframed),  # Solo 1 input (sin audio)
        "-vf",
        f"subtitles='{subtitle_file}'",
        "-c:v",
        "libx264",
        "-c:a",
        "aac",  # ← Esto falla porque no hay audio stream
        "-preset",
        "fast",
        "-crf",
        "23",
...
def test_ffmpeg_without_face_tracking():
    ...
    cmd = [
        ...
        str(start_time),
        "-i",
        str(video_original),  # Video con audio
        "-t",
        str(duration),
        # Filtros (crop + subtítulos)
        "-vf",
        f"crop=ih*9/16:ih,scale=1080:1920,subtitles='{subtitle_file}'",
