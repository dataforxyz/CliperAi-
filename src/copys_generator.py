                ...
                clips_data.append(
                    {
                        ...
                        "transcript": clip[
                            "full_text"
                        ],  # full_text contiene el transcript
                    }
                )
...
Responde SOLO con JSON válido (sin markdown):"""
                ...
                try:
                    ...
                    print(
                        f"   ⚠️  Skipping batch {batch_num}, continuing with next batch..."
                    )
                    continue
                except Exception as e:
                ...
                if isinstance(classification_result, list):
                    ...
                    batch_classifications = classification_result.get(
                        "classifications", []
                    )
                else:
                ...
                print(
                    f"   ✓ Extracted {len(batch_classifications)} classifications from batch"
                )
            ...
            if success_rate < 0.60:
                ...
                return {
                    ...
                    "logs": [
                        ...
                        f"   Clips faltantes: {sorted(missing)[:10]}...",  # Muestra primeros 10
                    ],
                }
            elif missing:
                # Algunos clips faltantes, pero suficientes para continuar
                logs.append(
                    f"⚠️  {len(missing)} clips no clasificados (continuando con {len(classified_ids)})"
                )
                logs.append(f"   Faltantes: {sorted(missing)}")
            ...
            return {"classifications": classifications, "logs": logs}

        except json.JSONDecodeError as e:
            return {
                "error_message": f"Error parseando JSON del classifier: {e}",
                "logs": [
                    "❌ JSON inválido del classifier",
    ...
    def group_by_style_node(self, state: CopysGeneratorState) -> dict:
        ...
        # Agrupar
        grouped = {"viral": [], "educational": [], "storytelling": []}
        ...
        if skipped > 0:
            print(
                f"   ⚠️  Skipped {skipped} classifications (clips not found in clips_data)"
            )
    ...
    def _generate_copies_for_style(
        self, clips: list[dict], style: str
    ) -> list[ClipCopy]:
        """
...
Responde SOLO con JSON válido (sin markdown):"""
            ...
            for attempt in range(3):
                try:
                    ...
                    if "clips" not in copies_data:
                        raise ValueError(
                            "La respuesta JSON no contiene la clave 'clips'"
                        )
                    ...
                    print(
                        f"   ✓ Batch {i//BATCH_SIZE + 1} successful on attempt {attempt + 1}"
                    )
                    break

                except (ValidationError, json.JSONDecodeError, ValueError) as e:
                    print(
                        f"   ❌ Attempt {attempt + 1}/3 FAILED for batch {i//BATCH_SIZE + 1}:"
                    )
                    ...
                    if attempt < 2:
                        ...
                        print("   ⚠️  Max retries reached. Skipping this batch.")
    ...
    def generate_viral_node(self, state: CopysGeneratorState) -> dict:
        ...
        try:
            ...
            if not viral_clips:
                return {"viral_copies": [], "logs": ["⏭️ Sin clips virales, saltando"]}
            ...
            return {
                ...
                "logs": [f"❌ Error viral: {e}", f"📋 Traceback: {error_detail[:500]}"],
            }
    ...
    def merge_results_node(self, state: CopysGeneratorState) -> dict:
        ...
        if len(all_copies_sorted) != total_clips:
            total_clips - len(all_copies_sorted)
            missing_ids = [
                clip["clip_id"]
                for clip in state["clips_data"]
    ...
    def validate_structure_node(self, state: CopysGeneratorState) -> dict:
        ...
        try:
            ...
            return {"logs": ["✅ Validación exitosa: todos los copies cumplen reglas"]}
    ...
    def analyze_quality_node(self, state: CopysGeneratorState) -> dict:
        ...
        if count == 0:
            return {
                ...
                "logs": [
                    ...
                    "   Verifica el modelo de Gemini y los logs anteriores",
                ],
            }
        ...
        if success_rate < MIN_SUCCESS_RATE:
            return {
                ...
                "logs": [
                    ...
                    "   Sugerencia: Espera 5 min y re-intenta (podría ser rate limiting 429)",
                ],
            }
        ...
        if count < total_clips:
            ...
            logs.append(
                f"⚠️  Generación parcial: {count}/{total_clips} copies ({success_rate:.0%})"
            )
            logs.append(
                f"   {missing_count} clips no pudieron generar copies (ver logs arriba)"
            )
        else:
        ...
        low_quality = [
            c.clip_id for c in all_copies if c.metadata.engagement_score < 6.5
        ]
    ...
    def save_results_node(self, state: CopysGeneratorState) -> dict:
        ...
        try:
            ...
            saved_data["generation_metadata"] = {
                ...
                "success_rate": (
                    round(generated_count / total_clips, 2) if total_clips > 0 else 0
                ),
                "missing_clips": missing_clips if is_incomplete else [],
                "note": (
                    "Partial generation - some batches failed during processing"
                    if is_incomplete
        ...
        return {
            ...
            "metrics": {
                ...
                "distribution": {
                    ...
                    "educational": len(
                        result.get("grouped_clips", {}).get("educational", [])
                    ),
                    "storytelling": len(
                        result.get("grouped_clips", {}).get("storytelling", [])
                    ),
                },
            },
